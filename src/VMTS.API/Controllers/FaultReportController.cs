...
public class FaultReportController : BaseApiController
{
    ...
    [HttpPost("create")]
    public async Task<ActionResult<FaultReportResponse>> CreateFaultReportAsync(
        FaultReportRequest request
    )
    {
        ...
        if (faultReport is null)
            return BadRequest(new ApiErrorResponse(400, "Failed to create fault report"));
